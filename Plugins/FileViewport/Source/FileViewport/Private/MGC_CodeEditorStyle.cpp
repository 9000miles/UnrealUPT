...
FString FMGC_CodeEditorStyle::InContent(const FString &RelativePath, const TCHAR* Extension) {
	static FString Content = IPluginManager::Get().FindPlugin(TEXT("FileViewport"))->GetContentDir();
...
void FMGC_CodeEditorStyle::Initialize() {
	...
	StyleSet->SetContentRoot(IPluginManager::Get().FindPlugin(TEXT("FileViewport"))->GetContentDir());
	//
	//
