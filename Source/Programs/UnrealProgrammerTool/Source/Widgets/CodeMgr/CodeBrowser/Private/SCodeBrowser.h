...
#include "Editor/CodeBrowser/Private/HistoryManager.h"
...
public:
	...
	void Construct( const FArguments& InArgs, const FName& InstanceName, const FCodeBrowserConfig* Config );

	//默认展开文件夹目录
	void OpenSourcesViewExpanded();
	 ...
	 */
	void SyncToAssets( const TArray<FFileData>& AssetDataList, const bool bAllowImplicitSync = false, const bool bDisableFiltersThatHideAssets = true );
	 ...
	 *								  false to force the view to select the explicit Parent folders of each asset 
	 */
	void SyncTo( const FCodeBrowserSelection& ItemSelection, const bool bAllowImplicitSync = false, const bool bDisableFiltersThatHideAssets = true );
	...
	void SetIsPrimaryCodeBrowser(bool NewIsPrimary);

	/** Returns if this browser can be used as the primary browser. */
	bool CanSetAsPrimaryCodeBrowser() const;
	...
	void GetSelectedAssets(TArray<FFileData>& SelectedAssets);
...
private:
	...
	void PrepareToSync( const TArray<FFileData>& AssetDataList, const TArray<FString>& FolderPaths, const bool bDisableFiltersThatHideAssets );
	...
	void OnAssetSelectionChanged(const FFileData& SelectedAsset);

	/** Handler for when the user double clicks, presses enter, or presses space on an asset */
	void OnAssetsActivated(const TArray<FFileData>& ActivatedAssets, EAssetTypeActivationMethod::Type ActivationMethod);
	...
	void OnFindInAssetTreeRequested(const TArray<FFileData>& AssetsToFind);

	/** Handler for when the user has committed a rename of an asset */
	void OnAssetRenameCommitted(const TArray<FFileData>& Assets);

	/** Handler for when the asset context menu requests to rename an asset */
	void OnRenameRequested(const FFileData& AssetData);
	...
	void OnOpenedFolderDeleted(TArray<FString>& SelectedPaths);
	...
	TSharedPtr<SWidget> GetFolderContextMenu(const TArray<FString>& SelectedPaths, FCodeBrowserMenuExtender_SelectedPaths InMenuExtender, FOnCreateNewFolder OnCreateNewFolder, bool bPathView);

	/** Delegate called to get the current selection state */
	void GetSelectionState(TArray<FFileData>& SelectedAssets, TArray<FString>& SelectedPaths);
