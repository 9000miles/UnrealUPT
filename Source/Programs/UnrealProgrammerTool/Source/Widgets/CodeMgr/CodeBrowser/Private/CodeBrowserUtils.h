...
namespace CodeBrowserUtils
{
	...
	bool CanRenameAsset(const FFileData& InAssetData);
	...
	void GetAssetsInPaths(const TArray<FString>& InPaths, TArray<FFileData>& OutAssetDataList);
	...
	void CopyAssetReferencesToClipboard(const TArray<FFileData>& AssetsToCopy);
	 ...
	 */
	void CaptureThumbnailFromViewport(FViewport* InViewport, const TArray<FFileData>& InAssetsToAssign);
	 ...
	 */
	void ClearCustomThumbnails(const TArray<FFileData>& InAssetsToAssign);

	/** Returns true if the specified asset that uses shared thumbnails has a thumbnail assigned to it */
	bool AssetHasCustomThumbnail( const FFileData& AssetData );
	...
	void GetObjectsInAssetData(const TArray<FFileData>& AssetList, TArray<UObject*>& OutDroppedObjects);
	...
	void CountItemTypes(const TArray<FFileData>& InItems, int32& OutNumAssetItems, int32& OutNumClassItems);
	...
	void BeginAdvancedCopyPackages(TArray<FFileData>& AssetList, TArray<FString>& AssetPaths, FString& DestinationPath);
}
