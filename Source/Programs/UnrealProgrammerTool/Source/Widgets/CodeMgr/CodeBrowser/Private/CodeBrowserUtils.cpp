...
class SCodeBrowserPopup : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS( SCodeBrowserPopup ){}
	...
	void Construct( const FArguments& InArgs )
	{
		...
		[
			...
			.Padding(10)
			.OnMouseButtonDown(this, &SCodeBrowserPopup::OnBorderClicked)
			.BorderBackgroundColor(this, &SCodeBrowserPopup::GetBorderBackgroundColor)
			[
				SNew(SHorizontalBox)
				...
				.Padding(0, 0, 4, 0)
				[
					SNew(SImage) .Image( FEditorStyle::GetBrush("CodeBrowser.PopupMessageIcon") )
				]
	...
	{
		TSharedRef<SCodeBrowserPopup> PopupContent = SNew(SCodeBrowserPopup) .Message(Message);
...
class SCodeBrowserConfirmPopup :  public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS( SCodeBrowserConfirmPopup ) {}
			...
			[
				...
				[
					...
					[
						...
						.OnClicked( this, &SCodeBrowserConfirmPopup::YesClicked )
					]
					...
					[
						...
						.HAlign(HAlign_Center)
						.Text(InArgs._NoText)
						.OnClicked( this, &SCodeBrowserConfirmPopup::NoClicked )
					]
				]
			]
		];
...
bool CodeBrowserUtils::OpenEditorForAsset(const FString& ObjectPath)
{
	// Load the asset if unloaded
	TArray<UObject*> LoadedObjects;
	TArray<FString> ObjectPaths;
	ObjectPaths.Add(ObjectPath);
	CodeBrowserUtils::LoadAssetsIfNeeded(ObjectPaths, LoadedObjects);
...
bool CodeBrowserUtils::OpenEditorForAsset(UObject* Asset)
{
	if( Asset != NULL )
	{
...
bool CodeBrowserUtils::OpenEditorForAsset(const TArray<UObject*>& Assets)
{
	if ( Assets.Num() == 1 )
	{
...
bool CodeBrowserUtils::LoadAssetsIfNeeded(const TArray<FString>& ObjectPaths, TArray<UObject*>& LoadedObjects, bool bAllowedToPromptToLoadAssets, bool bLoadRedirects)
{
	...
	if ( UnloadedObjectPaths.Num() > 0 )
	{
		...
		const bool bShowProgressDialog = (UnloadedObjectPaths.Num() > GetDefault<UCodeBrowserSettings>()->NumObjectsToLoadBeforeWarning) || bAtLeastOneUnloadedMap;
...
void CodeBrowserUtils::GetUnloadedAssets(const TArray<FString>& ObjectPaths, TArray<FString>& OutUnloadedObjects)
{
...
bool CodeBrowserUtils::PromptToLoadAssets(const TArray<FString>& UnloadedObjects)
{
...
bool CodeBrowserUtils::CanRenameFolder(const FString& InFolderPath)
{
	// Cannot rename folders that are part of a classes or collections root
	return !CodeBrowserUtils::IsClassPath(InFolderPath) && !CodeBrowserUtils::IsCollectionPath(InFolderPath);
}

bool CodeBrowserUtils::CanRenameAsset(const FFileData& InAssetData)
{
...
void CodeBrowserUtils::RenameAsset(UObject* Asset, const FString& NewName, FText& ErrorMessage)
{
...
void CodeBrowserUtils::CopyAssets(const TArray<UObject*>& Assets, const FString& DestPath)
{
...
void CodeBrowserUtils::MoveAssets(const TArray<UObject*>& Assets, const FString& DestPath, const FString& SourcePath)
{
...
int32 CodeBrowserUtils::DeleteAssets(const TArray<UObject*>& AssetsToDelete)
{
	return ObjectTools::DeleteObjects(AssetsToDelete);
}

bool CodeBrowserUtils::DeleteFolders(const TArray<FString>& PathsToDelete)
{
	...
	else
	{
		...
		if ( CodeBrowserUtils::LoadAssetsIfNeeded(ObjectPaths, LoadedAssets) )
		{
			...
			if ( LoadedAssets.Num() == NumAssetsInPaths )
			{
				...
				const int32 NumAssetsDeleted = CodeBrowserUtils::DeleteAssets(ToDelete);
...
bool CodeBrowserUtils::DeleteEmptyFolderFromDisk(const FString& InPathToDelete)
{
...
void CodeBrowserUtils::GetAssetsInPaths(const TArray<FString>& InPaths, TArray<FFileData>& OutAssetDataList)
{
...
bool CodeBrowserUtils::SavePackages(const TArray<UPackage*>& Packages)
{
...
bool CodeBrowserUtils::SaveDirtyPackages()
{
...
TArray<UPackage*> CodeBrowserUtils::LoadPackages(const TArray<FString>& PackageNames)
{
...
void CodeBrowserUtils::DisplayMessage(const FText& Message, const FSlateRect& ScreenAnchor, const TSharedRef<SWidget>& ParentContent)
{
	SCodeBrowserPopup::DisplayMessage(Message, ScreenAnchor, ParentContent);
}

void CodeBrowserUtils::DisplayConfirmationPopup(const FText& Message, const FText& YesString, const FText& NoString, const TSharedRef<SWidget>& ParentContent, const FOnClicked& OnYesClicked, const FOnClicked& OnNoClicked)
{
	TSharedRef<SCodeBrowserConfirmPopup> Popup = 
		SNew(SCodeBrowserConfirmPopup)
		.Prompt(Message)
		.YesText(YesString)
		.NoText(NoString)
...
bool CodeBrowserUtils::CopyFolders(const TArray<FString>& InSourcePathNames, const FString& DestPath)
{
...
bool CodeBrowserUtils::MoveFolders(const TArray<FString>& InSourcePathNames, const FString& DestPath)
{
	TMap<FString, TArray<UObject*> > SourcePathToLoadedAssets;
	...
	for ( auto PathIt = SourcePathToLoadedAssets.CreateConstIterator(); PathIt; ++PathIt )
	{
		...
		if ( PathIt.Value().Num() > 0 )
		{
			...
			CodeBrowserUtils::MoveAssets( PathIt.Value(), Destination, PathIt.Key() );
		}
...
bool CodeBrowserUtils::PrepareFoldersForDragDrop(const TArray<FString>& SourcePathNames, TMap< FString, TArray<UObject*> >& OutSourcePathToLoadedAssets)
{
...
void CodeBrowserUtils::CopyAssetReferencesToClipboard(const TArray<FFileData>& AssetsToCopy)
{
...
void CodeBrowserUtils::CaptureThumbnailFromViewport(FViewport* InViewport, const TArray<FFileData>& InAssetsToAssign)
{
	...
	if (InViewport->ReadPixels(OrigBitmap))
	{
		...
		{
			const FFileData& CurrentAsset = *AssetIt;
...
void CodeBrowserUtils::ClearCustomThumbnails(const TArray<FFileData>& InAssetsToAssign)
{
	FAssetToolsModule& AssetToolsModule = FModuleManager::LoadModuleChecked<FAssetToolsModule>("AssetTools");
...
bool CodeBrowserUtils::AssetHasCustomThumbnail( const FFileData& AssetData )
{
	FAssetToolsModule& AssetToolsModule = FModuleManager::LoadModuleChecked<FAssetToolsModule>("AssetTools");
...
CodeBrowserUtils::ECBFolderCategory CodeBrowserUtils::GetFolderCategory( const FString& InPath )
{
...
bool CodeBrowserUtils::IsEngineFolder( const FString& InPath )
{
...
bool CodeBrowserUtils::IsDevelopersFolder( const FString& InPath )
{
...
bool CodeBrowserUtils::IsPluginFolder(const FString& InPath, const TArray<TSharedRef<IPlugin>>& InPlugins, EPluginLoadedFrom* OutPluginSource)
{
...
bool CodeBrowserUtils::IsPluginFolder(const FString& InPath, EPluginLoadedFrom* OutPluginSource)
{
	return IsPluginFolder(InPath, IPluginManager::Get().GetEnabledPluginsWithContent(), OutPluginSource);
}

bool CodeBrowserUtils::IsClassesFolder(const FString& InPath)
{
...
bool CodeBrowserUtils::IsLocalizationFolder( const FString& InPath )
{
	return FPackageName::IsLocalizedPackage(InPath);
}

void CodeBrowserUtils::GetObjectsInAssetData(const TArray<FFileData>& AssetList, TArray<UObject*>& OutDroppedObjects)
{	
	for (int32 AssetIdx = 0; AssetIdx < AssetList.Num(); ++AssetIdx)
	{
		const FFileData& AssetData = AssetList[AssetIdx];
...
bool CodeBrowserUtils::IsValidFolderName(const FString& FolderName, FText& Reason)
{
...
bool CodeBrowserUtils::DoesFolderExist(const FString& FolderPath)
{
...
bool CodeBrowserUtils::IsEmptyFolder(const FString& FolderPath, const bool bRecursive)
{
	if (CodeBrowserUtils::IsClassPath(FolderPath))
	{
...
bool CodeBrowserUtils::IsRootDir(const FString& FolderPath)
{
	return IsAssetRootDir(FolderPath) || IsClassRootDir(FolderPath);
}

bool CodeBrowserUtils::IsAssetRootDir(const FString& FolderPath)
{
...
bool CodeBrowserUtils::IsClassRootDir(const FString& FolderPath)
{
...
FText CodeBrowserUtils::GetRootDirDisplayName(const FString& FolderPath)
{
	// Strip off any leading or trailing forward slashes
	// We just want the name without any path separators
...
bool CodeBrowserUtils::IsClassPath(const FString& InPath)
{
	static const FString ClassesRootPrefix = TEXT("/Classes_");
	return InPath.StartsWith(ClassesRootPrefix);
}

//bool CodeBrowserUtils::IsCollectionPath(const FString& InPath, FName* OutCollectionName, ECollectionShareType::Type* OutCollectionShareType)
//{
...
void CodeBrowserUtils::CountPathTypes(const TArray<FString>& InPaths, int32& OutNumAssetPaths, int32& OutNumClassPaths)
{
	OutNumAssetPaths = 0;
	OutNumClassPaths = 0;
...
void CodeBrowserUtils::CountPathTypes(const TArray<FName>& InPaths, int32& OutNumAssetPaths, int32& OutNumClassPaths)
{
	OutNumAssetPaths = 0;
	OutNumClassPaths = 0;
...
void CodeBrowserUtils::CountItemTypes(const TArray<FFileData>& InItems, int32& OutNumAssetItems, int32& OutNumClassItems)
{
	OutNumAssetItems = 0;
	OutNumClassItems = 0;

	for(const FFileData& Item : InItems)
	{
...
bool CodeBrowserUtils::IsValidPathToCreateNewClass(const FString& InPath)
{
...
bool CodeBrowserUtils::IsValidPathToCreateNewFolder(const FString& InPath)
{
...
const TSharedPtr<FLinearColor> CodeBrowserUtils::LoadColor(const FString& FolderPath)
{
	auto LoadColorInternal = [](const FString& InPath) -> TSharedPtr<FLinearColor>
	{
		...
		if(FPaths::FileExists(GEditorPerProjectIni))
		{
			...
			{
				FLinearColor Color;
				if(Color.InitFromString(ColorStr) && !Color.Equals(CodeBrowserUtils::GetDefaultColor()))
				{
			...
			{
				return PathColors.Add(InPath, MakeShareable(new FLinearColor(CodeBrowserUtils::GetDefaultColor())));
			}
		}
...
void CodeBrowserUtils::SaveColor(const FString& FolderPath, const TSharedPtr<FLinearColor>& FolderColor, bool bForceAdd)
{
	...
	const bool bRemove = !FolderColor.IsValid() || (!bForceAdd && FolderColor->Equals(CodeBrowserUtils::GetDefaultColor()));
...
bool CodeBrowserUtils::HasCustomColors( TArray< FLinearColor >* OutColors )
{
	...
	if (FPaths::FileExists(GEditorPerProjectIni))
	{
		...
		for( int32 SectionIndex = 0; SectionIndex < Section.Num(); SectionIndex++ )
		{
			...
			if ( EntryStr.Split( TEXT( "=" ), &PathStr, &ColorStr ) )
			{
				...
				if( CurrentColor.InitFromString( ColorStr ) && !CurrentColor.Equals( CodeBrowserUtils::GetDefaultColor() ) )
				{
					bHasCustom = true;
					if ( OutColors )
					{
...
FLinearColor CodeBrowserUtils::GetDefaultColor()
{
	// The default tint the folder should appear as
	return FLinearColor::Gray;
}

FText CodeBrowserUtils::GetExploreFolderText()
{
	FFormatNamedArguments Args;
...
bool CodeBrowserUtils::IsValidObjectPathForCreate(const FString& ObjectPath, FText& OutErrorMessage, bool bAllowExistingAsset)
{
...
bool CodeBrowserUtils::IsValidFolderPathForCreate(const FString& InFolderPath, const FString& NewFolderName, FText& OutErrorMessage)
{
	if (!CodeBrowserUtils::IsValidFolderName(NewFolderName, OutErrorMessage))
	{
		return false;
	}
	...
	if (CodeBrowserUtils::DoesFolderExist(NewFolderPath))
	{
	...
	const bool bDisplayL10N = GetDefault<UCodeBrowserSettings>()->GetDisplayL10NFolder();
	if (!bDisplayL10N && CodeBrowserUtils::IsLocalizationFolder(NewFolderPath))
	{
...
int32 CodeBrowserUtils::GetPackageLengthForCooking(const FString& PackageName, bool IsInternalBuild)
{
	...
	const bool bIsPluginAsset = CodeBrowserUtils::IsPluginFolder(PackageName, &PluginLoadedFrom);
	const bool bIsEngineAsset = CodeBrowserUtils::IsEngineFolder(PackageName) || (bIsPluginAsset && PluginLoadedFrom == EPluginLoadedFrom::Engine);
	...
	{
		UE_LOG(LogCodeBrowser, Error, TEXT("Package Name '%' is not a valid path and cannot be converted to a filename"), *PackageName);
	}
	return AbsoluteCookPathToAssetLength;
}

bool CodeBrowserUtils::IsValidPackageForCooking(const FString& PackageName, FText& OutErrorMessage)
{
...
void CodeBrowserUtils::SyncPackagesFromSourceControl(const TArray<FString>& PackageNames)
{
...
void CodeBrowserUtils::SyncPathsFromSourceControl(const TArray<FString>& ContentPaths)
{
	...
	if (PathsOnDisk.Num() > 0)
	{
		...
		TArray<FString> PackageNames;
		{
			...
			for (const FFileData& Asset : AssetList)
			{
		...
		UE_LOG(LogCodeBrowser, Log, TEXT("Syncing %d path(s):"), ContentPaths.Num());
		for (const UPackage* Package : LoadedPackages)
		{
			UE_LOG(LogCodeBrowser, Log, TEXT("\t - %s"), *Package->GetName());
		}
...
bool CodeBrowserUtils::CanDeleteFromAssetView(TWeakPtr<SAssetView> AssetView)
{
...
bool CodeBrowserUtils::CanRenameFromAssetView(TWeakPtr<SAssetView> AssetView)
{
	TArray< FFileData > AssetViewSelectedAssets = AssetView.Pin()->GetSelectedAssets();
	TArray< FString > SelectedFolders = AssetView.Pin()->GetSelectedFolders();

	const bool bOneAssetSelected = AssetViewSelectedAssets.Num() == 1 && SelectedFolders.Num() == 0		// A single asset
		&& CodeBrowserUtils::CanRenameAsset(AssetViewSelectedAssets[0]);								// Which can be renamed

	const bool bOneFolderSelected = AssetViewSelectedAssets.Num() == 0 && SelectedFolders.Num() == 1	// A single folder
		&& CodeBrowserUtils::CanRenameFolder(SelectedFolders[0]);									// Which can be renamed

	return (bOneAssetSelected || bOneFolderSelected) && !AssetView.Pin()->IsThumbnailEditMode();
}

bool CodeBrowserUtils::CanDeleteFromPathView(const TArray<FString>& SelectedPaths)
{
	int32 NumAssetPaths, NumClassPaths;
	CodeBrowserUtils::CountPathTypes(SelectedPaths, NumAssetPaths, NumClassPaths);

	// We can't delete folders containing classes
	bool bClassPath = NumAssetPaths > 0 && NumClassPaths == 0 ;
	bool bAssetPath = false;

#pragma region // 如果选中的路径是/Game/资源，则不能删除
	if (CHECK_ENGINE_IS_COURSEWARE_EDITOR == true)
	{
		if (SelectedPaths.Num() > 0)
			bAssetPath = SelectedPaths.ContainsByPredicate([](const FString& Path) {return Path.StartsWith(FString(TEXT("/Game/资源"))); });
	}
#pragma endregion

	return bClassPath && !bAssetPath;
}

bool CodeBrowserUtils::CanRenameFromPathView(const TArray<FString>& SelectedPaths)
{
	...
	if (CodeBrowserUtils::IsRootDir(SelectedPaths[0]))
	{
		return false;
	}

	// We can't rename *any* folders that belong to class roots
	if (CodeBrowserUtils::IsClassPath(SelectedPaths[0]))
	{
		return false;
	}

#pragma region // 如果选中的路径是/Game/课件， /Game/资源，则不能重命名
	if (CHECK_ENGINE_IS_COURSEWARE_EDITOR == true)
	{
		if (SelectedPaths[0] == FString(TEXT("/Game/课件"))
			|| SelectedPaths[0] == FString(TEXT("/Game/资源")))
		{
			return false;
		}
	}
...
bool CodeBrowserUtils::IsFavoriteFolder(const FString& FolderPath)
{
	return FCodeBrowserSingleton::Get().FavoriteFolderPaths.Contains(FolderPath);
}

void CodeBrowserUtils::AddFavoriteFolder(const FString& FolderPath, bool bFlushConfig /*= true*/)
{
	FCodeBrowserSingleton::Get().FavoriteFolderPaths.AddUnique(FolderPath);
}

void CodeBrowserUtils::RemoveFavoriteFolder(const FString& FolderPath, bool bFlushConfig /*= true*/)
{
	TArray<FString> FoldersToRemove;
	FoldersToRemove.Add(FolderPath);
	
	// Find and remove any subfolders
	for (const FString& FavoritePath : FCodeBrowserSingleton::Get().FavoriteFolderPaths)
	{
	...
	{
		FCodeBrowserSingleton::Get().FavoriteFolderPaths.Remove(FolderToRemove);
	}
	if (bFlushConfig)
	{
...
const TArray<FString>& CodeBrowserUtils::GetFavoriteFolders()
{
	return FCodeBrowserSingleton::Get().FavoriteFolderPaths;
}

int32 CodeBrowserUtils::GetMaxCookPathLen()
{
...
void CodeBrowserUtils::BeginAdvancedCopyPackages(TArray<FFileData>& AssetList, TArray<FString>& AssetPaths, FString& DestinationPath)
{
	TMap<FString, TArray<UObject*> > SourcePathToLoadedAssets;
