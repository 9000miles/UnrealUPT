...
typedef const FFileData& FAssetFilterType;
...
struct FAssetPickerConfig
{
	...
	FFileData InitialAssetSelection;
...
public:
	 ...
	 * @param	CodeBrowserConfig	Initial defaults for the new content browser, or nullptr to use saved settings
	 *
	 ...
	 */
	virtual TArray<FFileData> CreateModalOpenAssetDialog(const FOpenAssetDialogConfig& InConfig) = 0;
	...
	virtual bool HasPrimaryCodeBrowser() const = 0;

	/** Brings the primary content browser to the front or opens one if it does not exist. */
	virtual void FocusPrimaryCodeBrowser(bool bFocusSearch) = 0;
	...
	virtual void GetSelectedAssets(TArray<FFileData>& SelectedAssets) = 0;
	 ...
	 */
	virtual void CaptureThumbnailFromViewport(FViewport* InViewport, TArray<FFileData>& SelectedAssets) = 0;
