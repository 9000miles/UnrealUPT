...
DECLARE_DELEGATE_OneParam(FOnFindInAssetTreeRequested, const TArray<FFileData>& /*AssetsToFind*/);
...
DECLARE_DELEGATE_TwoParams(FOnAssetsActivated, const TArray<FFileData>& /*ActivatedAssets*/, EAssetTypeActivationMethod::Type /*ActivationMethod*/);
...
DECLARE_DELEGATE_OneParam(FOnAssetEnterPressed, const TArray<FFileData>& /*SelectedAssets*/);
...
DECLARE_DELEGATE_RetVal_OneParam(TSharedPtr<SWidget>, FOnGetAssetContextMenu, const TArray<FFileData>& /*SelectedAssets*/);
...
DECLARE_DELEGATE_TwoParams(FOnCodeBrowserGetSelection, TArray<FFileData>& /*SelectedAssets*/, TArray<FString>& /*SelectedPaths*/);
DECLARE_DELEGATE_TwoParams(FCodeBrowserCommandExtender, TSharedRef<FUICommandList> /*CommandList*/, FOnCodeBrowserGetSelection /*GetSelectionDelegate*/);

/** Called to request the menu when right clicking on a path */
DECLARE_DELEGATE_RetVal(TSharedRef<FExtender>, FCodeBrowserMenuExtender);
DECLARE_DELEGATE_RetVal_OneParam(TSharedRef<FExtender>, FCodeBrowserMenuExtender_SelectedAssets, const TArray<FFileData>& /*SelectedAssets*/);
DECLARE_DELEGATE_RetVal_OneParam(TSharedRef<FExtender>, FCodeBrowserMenuExtender_SelectedPaths, const TArray<FString>& /*SelectedPaths*/);

/** Called to request the menu when right clicking on an asset */
DECLARE_DELEGATE_RetVal_ThreeParams(TSharedPtr<SWidget>, FOnGetFolderContextMenu, const TArray<FString>& /*SelectedPaths*/, FCodeBrowserMenuExtender_SelectedPaths /*MenuExtender*/, FOnCreateNewFolder /*CreationDelegate*/);
...
DECLARE_DELEGATE_RetVal_TwoParams(FText, FOnGetCustomAssetColumnDisplayText, FFileData& /*AssetData*/, FName /*ColumnName*/);

/** Called to add extra asset data to the asset view, to display virtual assets. These get treated similar to Class assets */
DECLARE_DELEGATE_TwoParams(FOnGetCustomSourceAssets, const FARFilter& /*SourceFilter*/, TArray<FFileData>& /*AddedAssets*/);

/** Called to generate extra state information icons or tooltips on asset items. */
DECLARE_DELEGATE_RetVal_OneParam(TSharedRef<SWidget>, FOnGenerateAssetViewExtraStateIndicators, const FFileData& /*AssetData*/);

/** Called when an asset item visualizes its tooltip */
DECLARE_DELEGATE_RetVal_TwoParams(bool, FOnVisualizeAssetToolTip, const TSharedPtr<SWidget>& /*ToolTipContent*/, FFileData& /*AssetData*/);
